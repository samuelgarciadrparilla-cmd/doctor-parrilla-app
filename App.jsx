...
const PREMIUM_CSS = `
  ...
  .card-stagger { animation: card-enter 0.4s ease forwards; opacity: 0; }
  @keyframes timeline-light {
    from { opacity: 0; transform: translateX(-8px); }
    to   { opacity: 1; transform: translateX(0); }
  }
  .timeline-step { animation: timeline-light 0.4s ease forwards; opacity: 0; }
  @keyframes dot-pulse {
  ...
  @keyframes blur-in {
    ...
    to   { backdrop-filter: blur(12px); background: rgba(0,0,0,0.6); }
  }
  .blur-overlay { animation: blur-in 0.3s ease forwards; backdrop-filter: blur(12px); background: rgba(0,0,0,0.6); }
  ...
  .slide-in-right { animation: slide-in-right 0.28s cubic-bezier(0.25,0.46,0.45,0.94) forwards; }
  .slide-in-left  { animation: slide-in-left  0.28s cubic-bezier(0.25,0.46,0.45,0.94) forwards; }
  .sparkline-path { stroke-dasharray: 200; stroke-dashoffset: 200; animation: sparkline-draw 1.2s ease forwards 0.3s; }
  ...
  .shimmer-text {
    background: linear-gradient(90deg, #9A7A3E 0%, #E2C98A 40%, #C8A96E 60%, #9A7A3E 100%);
  ...
  @keyframes ember-rise {
    ...
    40%  { transform: translateY(-35px) scale(.75) rotate(12deg) translateX(6px); opacity: .55; }
    100% { transform: translateY(-90px) scale(.2) rotate(-8deg) translateX(-4px); opacity: 0; }
  }
  @keyframes orb-float {
    0%,100% { transform: translate(0,0) scale(1); opacity: 0.08; }
    33%      { transform: translate(15px,-10px) scale(1.08); opacity: 0.14; }
    66%      { transform: translate(-10px,8px) scale(0.94); opacity: 0.06; }
  }
  ...
  .progress-wave {
    background: linear-gradient(90deg, #9A7A3E, #E2C98A, #C8A96E, #9A7A3E);
    background-size: 200% 100%;
    animation: progress-wave 2s linear infinite;
  }
  @keyframes kpi-glow {
    0%, 100% { box-shadow: 0 0 8px rgba(200,169,110,0.2); }
    50%       { box-shadow: 0 0 22px rgba(200,169,110,0.45); }
  }
  .kpi-card { animation: kpi-glow 3s ease-in-out infinite; }
  @keyframes count-up {
    from { opacity: 0; transform: scale(0.85); }
    to   { opacity: 1; transform: scale(1); }
  }
  .counter-animate { animation: count-up 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards; }
  ...
  @keyframes ripple-btn { to { transform: scale(4); opacity: 0; } }
`;
...
// ── EMBER PARTICLE SYSTEM (v2 physics) ──────────────────────────────────────
const T_EMBER = {
  spark:"#F07830", flame:"#E05010", amber:"#E8C472", gold:"#D4A84B",
};
function EmberSystem({ count = 14, bottom = 0, spread = 90 }) {
  const particles = Array.from({length:count}, (_,i) => ({
    id: i,
    left: `${5 + (i / count) * spread}%`,
    size: 1.5 + (i % 4) * 0.8,
    delay: `${(i * 0.28) % 2.5}s`,
    dur: `${1.8 + (i % 4) * 0.7}s`,
    color: [T_EMBER.spark, T_EMBER.flame, T_EMBER.amber, T_EMBER.gold][i % 4],
  }));
  return (
    <div style={{position:"absolute",bottom,left:0,right:0,height:90,pointerEvents:"none",overflow:"hidden",zIndex:0}}>
      {particles.map(p => (
        <div key={p.id} style={{
          ...
          animation:`ember-rise ${p.dur} ${p.delay} ease-in infinite`,
        }}/>
      ))}
    </div>
  );
...
// ── AMBIENT ORB ───────────────────────────────────────────────────────────────
function AmbientOrb({x,y,size,color,delay="0s"}) {
  return (
    <div style={{
      position:"absolute", left:x, top:y,
      width:size, height:size, borderRadius:"50%",
      background:`radial-gradient(circle, ${color} 0%, transparent 70%)`,
      animation:`orb-float 8s ${delay} ease-in-out infinite`,
      pointerEvents:"none",
    }}/>
  );
}

// ── CONFETTI (Duolingo style) ─────────────────────────────────────────────────
const CONFETTI_COLORS = ["#C8A96E","#E2C98A","#FF6B35","#FFD700","#FF4444","#44FF88","#4488FF","#FF44FF"];
function ConfettiCelebration({ active }) {
  const [pieces, setPieces] = _useState([]);
  _useEffect(() => {
    if (!active) return;
    ...
    <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, pointerEvents:"none", zIndex:9999, overflow:"hidden" }}>
      {pieces.map(p => (
        <div key={p.id} style={{
          ...
          animation:`confetti-fall ${p.duration}s ease-in ${p.delay}s forwards, confetti-sway 1s ease-in-out ${p.delay}s infinite`,
        }} />
      ))}
    </div>
  );
...
// ── SPARKLINE (Stripe style) ──────────────────────────────────────────────────
function Sparkline({ data = [], color = "#C8A96E", width = 70, height = 24 }) {
  ...
  return (
    <svg width={width} height={height} style={{ overflow:"visible" }}>
      ...
      <polyline points={pts} fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
        className="sparkline-path" style={{ filter:`drop-shadow(0 0 3px ${color}66)` }} />
    </svg>
  );
}

// ── ANIMATED COUNTER ─────────────────────────────────────────────────────────
function AnimatedCounter({ value, duration = 1000, suffix = "", prefix = "", style = {} }) {
  const [display, setDisplay] = _useState(0);
  const rafRef = _useRef ? _useRef(null) : { current: null };
  _useEffect(() => {
  ...
  return <span style={style} className="counter-animate">{prefix}{display.toLocaleString("es-PY")}{suffix}</span>;
}

// ── DOTS LOADING (Spotify style) ─────────────────────────────────────────────
function DotsLoading({ color = "#C8A96E", size = 8 }) {
  return (
    <div style={{ display:"flex", gap:size*0.6, alignItems:"center", justifyContent:"center", padding:"8px 0" }}>
      {[1,2,3].map(i => <div key={i} className={`dot-${i}`} style={{ width:size, height:size, borderRadius:"50%", background:color }} />)}
    </div>
  );
}

// ── KPI CARD (Stripe style) ───────────────────────────────────────────────────
function KpiCard({ label, value, trend, trendUp, sparkData, color = "#C8A96E", delay = "0s" }) {
  return (
    <div className="kpi-card card-stagger" style={{ animationDelay:delay, background:"#0F1115", border:`1px solid ${color}33`, borderRadius:12, padding:"14px 16px", position:"relative", overflow:"hidden" }}>
      <div style={{ position:"absolute", top:0, right:0, width:60, height:60, background:`radial-gradient(circle at top right, ${color}15, transparent)`, pointerEvents:"none" }} />
      <div style={{ fontSize:9, color:"#555", fontFamily:"sans-serif", letterSpacing:"2px", marginBottom:6, textTransform:"uppercase" }}>{label}</div>
      <div style={{ fontSize:20, fontWeight:"bold", color, fontFamily:"'DM Mono', monospace", lineHeight:1, marginBottom:4 }}>{value}</div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
        {trend && <div style={{ fontSize:10, color:trendUp?"#4CAF50":"#E57373", fontFamily:"sans-serif" }}>{trendUp?"↑":"↓"} {trend}</div>}
        {sparkData && <Sparkline data={sparkData} color={color} width={60} height={18} />}
      </div>
    </div>
  );
}
...
[
"Tu pedido está en nuestras manos. Los mejores artesanos del acero ya conocen tu nombre. Seguí el progreso en drparrillaparaguay.com ingresando con tu número de teléfono.",
"Acabamos de recibir tu pedido. Nuestro equipo ya está planificando cada detalle de tu pieza única. Podés ver el avance en drparrillaparaguay.com con tu número de teléfono.",
"Tu proyecto acaba de entrar al taller. Pronto el acero tomará la forma de tus sueños. Seguí cada etapa en drparrillaparaguay.com.",
"Pedido confirmado. Cada obra maestra comienza con un primer paso - el tuyo ya está en marcha. Seguí el progreso en drparrillaparaguay.com con tu número de teléfono.",
"Bienvenido al proceso. Tu parrilla será creada con la misma pasión con la que vos disfrutás un buen asado. Seguí el avance en drparrillaparaguay.com.",
"El acero ya te espera. Nuestros maestros soldadores están preparando todo para tu pieza exclusiva. Monitoreá el progreso en drparrillaparaguay.com.",
],
// Estado 1: En producción
[
"Manos expertas forjan tu parrilla ahora mismo. Cada corte, cada soldadura - hecho para toda la vida. Seguí el progreso en drparrillaparaguay.com con tu número de teléfono.",
"El fuego del taller arde por tu pedido. Acero, precisión y pasión se unen en cada centímetro. Mirá el avance en drparrillaparaguay.com.",
"Tu parrilla está tomando forma. Cada soldadura es un compromiso de calidad que dura generaciones. Seguí cada etapa en drparrillaparaguay.com.",
"Ahora mismo, manos artesanas dan vida al acero. Tu parrilla no se fabrica - se crea. Seguí el progreso en drparrillaparaguay.com con tu número de teléfono.",
"El taller vibra con tu pedido. Corte, doblado, soldadura - cada paso es perfección calculada. Mirá el avance en drparrillaparaguay.com.",
"Tu pieza está en el corazón del proceso. Solo usamos acero premium porque tu asado lo merece. Seguí el progreso en drparrillaparaguay.com.",
"Artesanos con décadas de experiencia trabajan en tu pedido. No es producción en serie - es arte. Seguí cada etapa en drparrillaparaguay.com con tu número de teléfono.",
"El acero inoxidable 304 ya tiene tu nombre. Cada milímetro se mide, se corta, se perfecciona. Mirá el avance en drparrillaparaguay.com.",
],
// Estado 2: Control de calidad
[
"Tu parrilla pasó por control de calidad. Solo lo perfecto llega a tu hogar. Seguí el progreso en drparrillaparaguay.com con tu número de teléfono.",
"Inspección final en curso. Revisamos cada soldadura, cada acabado, cada detalle. Cero defectos. Mirá el avance en drparrillaparaguay.com.",
"Tu parrilla está siendo examinada con lupa. Nuestro estándar es la perfección - sin excepciones. Seguí cada etapa en drparrillaparaguay.com.",
"Control de calidad premium: cada unión, cada superficie, cada ángulo debe ser impecable. Seguí el progreso en drparrillaparaguay.com con tu número de teléfono.",
"Estamos verificando que tu pieza cumpla con los estándares europeos de calidad que nos definen. Mirá el avance en drparrillaparaguay.com.",
"Tu parrilla casi está lista. Solo falta confirmar que cada detalle esté a la altura de tu expectativa. Seguí el progreso en drparrillaparaguay.com.",
],
// Estado 3: Lista para entrega
[
"¡Tu parrilla de ensueño está lista! Pronto el aroma del asado llenará tu espacio. Seguí el progreso en drparrillaparaguay.com con tu número de teléfono.",
"Obra maestra terminada. Tu parrilla espera el momento de brillar en tu hogar. Mirá el avance en drparrillaparaguay.com.",
"El acero ya tiene alma - la tuya. Tu parrilla está lista para escribir historias inolvidables. Seguí cada etapa en drparrillaparaguay.com.",
"Preparando el embalaje premium. Tu pieza viaja protegida como la joya que es. Seguí el progreso en drparrillaparaguay.com con tu número de teléfono.",
"Tu parrilla está lista y ansiosa por conocer su nuevo hogar. El primer asado será legendario. Mirá el avance en drparrillaparaguay.com.",
"Misión cumplida en el taller. Ahora solo falta que el fuego y la carne hagan su magia. Seguí el progreso en drparrillaparaguay.com.",
"¿Ya compraste la carne? Estamos cerca de entregarte tu parrilla. ¡Prepará un buen vacío para el estreno! Seguí el avance en drparrillaparaguay.com con tu número de teléfono.",
"¡No te olvides del carbón! Quebracho o algarrobo son los mejores para inaugurar tu Dr. Parrilla. Mirá el progreso en drparrillaparaguay.com.",
"¿Un buen vino o una cerveza bien fría para maridar los cortes de carne? El momento perfecto se acerca. Seguí el avance en drparrillaparaguay.com.",
"Comprá la carne, prepará la mesa, invitá a la familia. Tu Dr. Parrilla está a punto de llegar. Seguí el progreso en drparrillaparaguay.com con tu número de teléfono.",
"¿Ya tenés el chimichurri listo? Tu parrilla está esperando su gran debut. Mirá el avance en drparrillaparaguay.com.",
"Consejo: un buen sal gruesa y leña de quebracho. Tu primera brasa será inolvidable. Seguí el progreso en drparrillaparaguay.com.",
],
// Estado 4: Entregada
[
"¡Bienvenida a tu familia! Que cada asado sea un recuerdo imborrable. 🎁 ¿Dejás una reseña en Google? Te regalamos un cupón de descuento en tu próxima compra. Ingresá a drparrillaparaguay.com con tu número de teléfono para reclamarlo.",
"Tu parrilla ya está en casa. Cada asado que hagas será una celebración de la vida. 🎁 Dejá tu reseña en Google y recibí un cupón de descuento especial. Ingresá a drparrillaparaguay.com con tu número de teléfono.",
"Entregada con orgullo. Cuidala y ella te dará décadas de asados perfectos. 🎁 ¿Nos dejás una reseña en Google? Te regalamos un cupón de descuento en tu próxima compra. Reclamalo en drparrillaparaguay.com.",
"Tu inversión en calidad ya rinde frutos. Disfrutá cada momento alrededor del fuego. 🎁 Dejá tu reseña en Google y recibí un cupón de descuento exclusivo. Ingresá a drparrillaparaguay.com con tu número de teléfono.",
"La parrilla llegó a destino. Ahora empieza la verdadera historia - la que escribís vos con cada asado. 🎁 ¿Nos dejás una reseña en Google? Te regalamos un cupón de descuento. Reclamalo en drparrillaparaguay.com.",
"Gracias por confiar en Dr. Parrilla. Tu satisfacción es nuestro mayor logro. 🎁 Dejá tu reseña en Google y recibí un cupón de descuento en tu próxima compra. Ingresá a drparrillaparaguay.com con tu número de teléfono. #ElFuegoNosUne🔥",
],
];
...
// ── PDF UPLOAD BUTTON ───────────────────────────────────────────────────────
function PdfUploadButton({ onPdf, label = "📄 Subir PDF", style = {} }) {
const ref = useRef();
const [uploading, setUploading] = useState(false);
...
return (
<>
<input type="file" accept="application/pdf" ref={ref} onChange={handle} style={{ display:"none" }} />
<button onClick={() => !uploading && ref.current?.click()}
style={{ background:CARD, border:`1px solid ${uploading?GOLD:BORDER}`, color:uploading?GOLD:"#CCC", padding:"12px 16px", borderRadius:10, fontFamily:"sans-serif", fontSize:13, cursor:uploading?"wait":"pointer", display:"flex", alignItems:"center", gap:8, opacity:uploading?0.8:1, ...style }}>
{uploading ? `⏳ ${progress}` : (progress || label)}
</button>
</>
);
}
...
// NO se crea el cupón aquí — solo se crea cuando el cliente toca el botón de Google
setStep(3);
};

const crearCuponPendiente = () => {
if (setCupones) {
...
// Verificar que no existe ya un cupón pendiente para este pedido
const yaExiste = (cupones||[]).some(c => c.motivo && c.motivo.includes(pedido.id));
if (!yaExiste) {
setCupones(prev => [{
id: cuponCode, cliente: pedido.tel, clienteTel: pedido.tel,
descuento: 10, motivo: "Reseña en Google - " + pedido.modelo + " (" + pedido.id + ")",
estado: "pendiente", fecha, vence, tipo: "resena", duracionMeses: 3
}, ...(prev||[])]);
}
}
...
Dejá tu reseña en Google y recibí un <span style={{ color:GOLD, fontWeight:"bold" }}>cupón de 10% de descuento</span> en servicios, mantenimiento o nuevos productos.
</div>
...
<div style={{ background:"#1A0800", border:`1px solid ${GOLD}33`, borderRadius:8, padding:"10px 12px", marginBottom:8 }}>
<div style={{ fontSize:12, color:"#E8A020", fontFamily:"sans-serif", fontWeight:"bold", marginBottom:2 }}>⏳ Pendiente de validación</div>
<div style={{ fontSize:11, color:"#888", fontFamily:"sans-serif", lineHeight:1.5 }}>Nuestro equipo verificará tu reseña en Google y activará el cupón en menos de 24hs. Te notificaremos por WhatsApp.</div>
</div>
...
<button onClick={crearCuponPendiente}
style={{ display:"block", width:"100%", background:"linear-gradient(135deg,#4285F4,#34A853)", border:"none", color:"#FFF", padding:"14px", borderRadius:10, fontSize:15, fontFamily:"sans-serif", fontWeight:"bold", cursor:"pointer", textAlign:"center", boxSizing:"border-box", marginBottom:10 }}>
⭐ Ir a Google y dejar mi reseña
</button>
...
<div style={{ background:"linear-gradient(145deg, rgba(15,17,21,0.95), rgba(10,12,16,0.98))", border:`1px solid ${GOLD}28`, borderRadius:18, padding:"20px", marginBottom:16, position:"relative", overflow:"hidden", boxShadow:`0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(212,168,75,0.08)` }}>
  {/* Ambient glow */}
  <div style={{ position:"absolute", top:-30, right:-30, width:120, height:120, borderRadius:"50%", background:`radial-gradient(circle, ${GOLD}12, transparent)`, pointerEvents:"none" }}/>

  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
    <div style={{ width:3, height:18, borderRadius:2, background:GOLD_GRAD }}/>
    <div style={{ fontSize:9, color:GOLD, fontFamily:"sans-serif", letterSpacing:"3px", fontWeight:"bold" }}>🔥 TIPS DEL ASADOR</div>
  </div>

  <div style={{ display:"flex", gap:14, alignItems:"flex-start", marginBottom:18 }}>
    <div style={{ width:52, height:52, borderRadius:14, background:`${GOLD}12`, border:`1px solid ${GOLD}25`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:26, flexShrink:0, boxShadow:`0 4px 16px rgba(0,0,0,0.3)` }}>
      {tip.icon}
    </div>
    <div style={{ flex:1 }}>
      <div style={{ fontSize:16, fontWeight:"bold", color:CREAM, marginBottom:8, lineHeight:1.2, letterSpacing:"-0.3px" }}>{tip.titulo}</div>
      <div style={{ fontSize:13, color:"#C0B090", fontFamily:"sans-serif", lineHeight:1.75, letterSpacing:"0.1px" }}>{tip.texto}</div>
    </div>
  </div>

  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", paddingTop:14, borderTop:`1px solid ${GOLD}15` }}>
    <div style={{ display:"flex", gap:6 }}>
      {TIPS.map((_,i) => <div key={i} onClick={() => setTipIdx(i)} style={{ width:i===tipIdx?22:6, height:5, borderRadius:3, background:i===tipIdx?GOLD:BORDER2, cursor:"pointer", transition:"all 0.25s", boxShadow:i===tipIdx?`0 0 8px ${GOLD}66`:"none" }}/>)}
    </div>
    <button onClick={() => setTipIdx((tipIdx+1)%TIPS.length)} style={{ background:`${GOLD}18`, border:`1px solid ${GOLD}35`, color:GOLD, padding:"7px 16px", borderRadius:20, fontSize:11, fontFamily:"sans-serif", cursor:"pointer", letterSpacing:"1px", fontWeight:"bold", transition:"all .2s" }}>
      Siguiente →
    </button>
  </div>
</div>
...
<div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:14 }}>
  <div style={{ width:3, height:18, borderRadius:2, background:GOLD_GRAD }}/>
  <div style={{ fontSize:9, color:GOLD, fontFamily:"sans-serif", letterSpacing:"3px", fontWeight:"bold" }}>🔥 ¿SABÍAS TODO LO QUE PODÉS COCINAR?</div>
</div>
<div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
{items.map((item, i) => (
<div key={i} style={{ background:"linear-gradient(145deg, rgba(15,17,21,0.95), rgba(10,12,16,0.98))", border:`1px solid ${GOLD}20`, borderRadius:14, padding:"16px 12px", textAlign:"center", boxShadow:`0 4px 16px rgba(0,0,0,0.3)`, transition:"transform .2s, box-shadow .2s" }}>
<div style={{ width:48, height:48, borderRadius:12, background:`${GOLD}10`, border:`1px solid ${GOLD}20`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:24, margin:"0 auto 10px" }}>{item.emoji}</div>
<div style={{ fontSize:12, fontWeight:"bold", color:GOLD_LIGHT, fontFamily:"sans-serif", marginBottom:6, letterSpacing:"0.3px" }}>{item.titulo}</div>
<div style={{ fontSize:11.5, color:"#A09070", fontFamily:"sans-serif", lineHeight:1.6 }}>{item.desc}</div>
</div>
))}
</div>
...
<div style={{ background:`linear-gradient(145deg, ${color}0A, rgba(10,12,16,0.98))`, border:`1px solid ${color}35`, borderRadius:18, padding:"22px 18px", marginBottom:20, position:"relative", overflow:"hidden", boxShadow:`0 8px 32px rgba(0,0,0,0.4), 0 0 0 1px ${color}10` }}>
  {/* Ember particles when active */}
  {estado > 0 && estado < 4 && <EmberSystem count={6} bottom={0} spread={80}/>}
    ...
    <div style={{ flexShrink:0, position:"relative", width:120, height:120 }}>
      <svg width="120" height="120" style={{ transform:"rotate(-90deg)" }}>
        ...
        <circle cx="60" cy="60" r={r} fill="none" stroke={color} strokeWidth="10"
          ...
          strokeLinecap="round"
          style={{ transition:"stroke-dashoffset 1.2s cubic-bezier(0.4,0,0.2,1)", filter:`drop-shadow(0 0 8px ${color}AA)` }}/>
      </svg>
      <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", textAlign:"center" }}>
        <div style={{ fontSize:26, fontWeight:"900", color, lineHeight:1, fontFamily:"'DM Mono', monospace", textShadow:`0 0 20px ${color}66` }}>
          <AnimatedCounter value={pct} duration={1400} suffix="%"/>
        </div>
        <div style={{ fontSize:8, color:"#666", fontFamily:"sans-serif", marginTop:3, letterSpacing:"2px", textTransform:"uppercase" }}>avance</div>
      </div>
    </div>

    {/* Info */}
    <div style={{ flex:1 }}>
      <div style={{ fontSize:9, color:"#555", fontFamily:"sans-serif", letterSpacing:"2px", marginBottom:6, textTransform:"uppercase" }}>Estado actual</div>
      <div style={{ fontSize:16, fontWeight:"bold", color, marginBottom:10, lineHeight:1.2 }}>{ESTADO_LABELS[estado]}</div>
      {diasRestantes !== null && (
        <div style={{ display:"flex", alignItems:"center", gap:8, background:`${color}10`, border:`1px solid ${color}25`, borderRadius:10, padding:"8px 12px" }}>
          <span style={{ fontSize:18 }}>{retrasado?"🚨":urgente?"⏳":"📅"}</span>
          <div>
            <div style={{ fontSize:13, color, fontFamily:"'DM Mono', monospace", fontWeight:"bold", lineHeight:1 }}>
              {retrasado ? `${Math.abs(diasRestantes)}d retraso` : diasRestantes === 0 ? "¡Hoy!" : `${diasRestantes}d hábiles`}
            </div>
            <div style={{ fontSize:9, color:"#666", fontFamily:"sans-serif", marginTop:2 }}>para entrega</div>
          </div>
        </div>
      )}
    </div>
  ...
  {/* Mensaje manos expertas - mejorado */}
  {estado > 0 && estado < 4 && (
    <div style={{ marginTop:16, paddingTop:14, borderTop:`1px solid ${color}18`, position:"relative", zIndex:1 }}>
      <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
        <div style={{ fontSize:20, flexShrink:0, marginTop:1 }}>⚒️</div>
        <div>
          <div style={{ fontSize:11, color:GOLD, fontFamily:"sans-serif", fontWeight:"bold", marginBottom:4, letterSpacing:"0.5px" }}>Manos expertas forjan tu parrilla</div>
          <div style={{ fontSize:12, color:"#A09070", fontFamily:"sans-serif", lineHeight:1.7, fontStyle:"italic" }}>
            Cada corte, cada soldadura — hecho para toda la vida.
          </div>
        </div>
      </div>
    </div>
  ...
  {mensajeRetraso && (
    <div style={{ marginTop:14, background: retrasado?"rgba(26,0,0,0.8)":"rgba(26,8,0,0.8)", border:`1px solid ${color}44`, borderRadius:12, padding:"14px 16px", fontSize:12, color:"#D0C0A0", fontFamily:"sans-serif", lineHeight:1.7, position:"relative", zIndex:1 }}>
      <div style={{ fontWeight:"bold", color, marginBottom:4 }}>{retrasado?"⚠️ Aviso importante":"🔥 ¡Casi listo!"}</div>
      {mensajeRetraso}
    </div>
  )}
</div>
...
<div style={{ minHeight:"100vh", background:STEEL_BG, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 24px", position:"relative", overflow:"hidden" }}>
  ...
  {tab==="login" ? (
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <div style={{ position:"relative" }}>
            <input type={showPass?"text":"password"} placeholder="••••••••" value={pass}
              ...
              onKeyDown={e => e.key==="Enter" && handleLogin()}
              autoFocus
              style={{ width:"100%", background:"rgba(255,255,255,0.03)", border:`1px solid ${GOLD}55`, color:CREAM, padding:"14px 48px 14px 16px", borderRadius:12, fontSize:16, fontFamily:"sans-serif", outline:"none", boxSizing:"border-box" }}/>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:10, justifyItems:"center", marginBottom:14 }}>
        {[{f:"🇵🇾",n:"Paraguay"},{f:"🇦🇷",n:"Argentina"},{f:"🇧🇷",n:"Brasil"},{f:"🇺🇾",n:"Uruguay"},{f:"🇨🇴",n:"Colombia"},{f:"🇺🇸",n:"EE.UU."},{f:"🇪🇸",n:"España"},{f:"🇵🇦",n:"Panamá"},{f:"🇸🇻",n:"El Salvador"},{f:"🇷🇸",n:"Serbia"}].map((p,i) => (
          <div key={i} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
            <div style={{ width:38, height:38, borderRadius:"50%", background:"rgba(255,255,255,0.04)", border:`1px solid ${BORDER2}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, transition:"transform .2s" }}>{p.f}</div>
    ...
    <div style={{ fontSize:7, color:"#2A2A2A", fontFamily:"sans-serif", marginTop:24, marginBottom:8, textAlign:"center", letterSpacing:2, opacity:0.6 }}>Dr. Parrilla · v3.0 · #ElFuegoNosUne 🔥</div>
  </div>
</div>
);
}
...
<EmberSystem count={5} bottom={0} spread={90}/>
...
// ── CLIENT: CATÁLOGO ────────────────────────────────────────────────
function CatalogoScreen({ productos, config }) {
...
{productos.map((p, _pi) => (
<button key={p.id} onClick={() => setSelected(p.id)} className="card-stagger" style={{ animationDelay:`${_pi*0.07}s`, background:CARD, border:`1px solid ${BORDER}`, borderRadius:12, padding:0, cursor:"pointer", textAlign:"left", overflow:"hidden", position:"relative" }}>
<EmberSystem count={2} bottom={0} spread={70}/>
...
{/* ── PROGRESS HERO PREMIUM (v2 style) ── */}
<div style={{
  background:`linear-gradient(148deg, rgba(26,8,0,0.9) 0%, rgba(13,14,22,0.92) 100%)`,
  border:`1px solid ${GOLD}44`,
  borderRadius:22, padding:24, marginBottom:16,
  position:"relative", overflow:"hidden",
  boxShadow:`0 20px 48px rgba(0,0,0,.4)`,
}}>
  <EmberSystem count={6} bottom={0} spread={85}/>
  <AmbientOrb x="60%" y="-30px" size="140px" color="rgba(212,168,75,0.14)"/>

  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:16, position:"relative" }}>
    <div>
      <div style={{ fontSize:9, color:GOLD_DARK, fontFamily:"'DM Mono',monospace", letterSpacing:"3px", marginBottom:8 }}>PROGRESO TOTAL</div>
      <div style={{ display:"flex", alignItems:"baseline", gap:3 }}>
        <span style={{ fontSize:52, fontWeight:900, fontFamily:"'DM Mono',monospace", lineHeight:1, color:GOLD_LIGHT }}>
          <AnimatedCounter value={pct} duration={1400}/>
        </span>
        <span style={{ fontSize:22, color:GOLD, fontFamily:"'DM Mono',monospace" }}>%</span>
      </div>
    </div>
    {lim && (
      <div style={{ textAlign:"right" }}>
        <div style={{ fontSize:9, color:"rgba(242,223,186,0.4)", fontFamily:"'DM Mono',monospace", marginBottom:4 }}>ENTREGA EST.</div>
        <div style={{ fontSize:16, color:CREAM, fontFamily:"'DM Mono',monospace", fontWeight:600 }}>{formatFechaCorta(lim)}</div>
        <div style={{ fontSize:10, color:GOLD, fontFamily:"'DM Mono',monospace" }}>{p.diasHabiles} días hábiles</div>
      </div>
    )}
  </div>

  {/* Barra con wave */}
  <div style={{ height:8, background:"rgba(255,255,255,0.05)", borderRadius:8, overflow:"hidden", marginBottom:14, position:"relative" }}>
    <div className="progress-wave" style={{ width:`${pct}%`, height:"100%", borderRadius:8, boxShadow:`0 0 20px rgba(212,168,75,.7)`, position:"relative", overflow:"hidden", transition:"width 1.4s cubic-bezier(0.4,0,0.2,1)" }}>
      <div style={{ position:"absolute", inset:0, background:`linear-gradient(90deg,transparent 0%,rgba(255,255,255,.3) 50%,transparent 100%)`, animation:"wave 1.8s ease-in-out infinite" }}/>
    </div>
  </div>

  {/* 5-step mini indicators */}
  <div style={{ display:"flex", gap:4, marginBottom:14 }}>
    {ESTADO_LABELS.map((_,i) => (
      <div key={i} style={{ flex:1, height:3, borderRadius:3,
        background: i<=p.estado ? (i===p.estado ? `linear-gradient(135deg,${GOLD},${GOLD_LIGHT})` : `${GOLD}55`) : "rgba(255,255,255,0.06)",
        boxShadow: i===p.estado ? `0 0 8px rgba(212,168,75,.6)` : "none",
        transition:"all .3s"
      }}/>
    ))}
  </div>
  ...
  {/* Neuro message */}
  <div style={{ borderTop:`1px solid ${GOLD}18`, paddingTop:12, fontSize:12, color:"rgba(242,223,186,0.55)", fontFamily:"Georgia,serif", fontStyle:"italic", lineHeight:1.75, position:"relative" }}>
    {getNeuroMessage(p.estado)}
  </div>
</div>
...
{p.estado === 3 && <ConfettiCelebration active={true} />}
{ESTADO_LABELS.map((est,i) => {
const done=i<=p.estado; const act=i===p.estado;
return (
<div key={i} className="timeline-step" style={{ animationDelay:`${i*0.1}s`, display:"flex", gap:16 }}>
...
const _sparkData = [0,1,2,3,4].map(s => s <= p.estado ? (s+1)*20 : 0);
return (
<button key={p.id} onClick={() => setSelected(i)} className="card-stagger" style={{ animationDelay:`${i*0.1}s`, background:CARD,border:`1px solid ${BORDER}`,borderRadius:12,padding:"18px",cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden" }}>
...
{/* Redes sociales - debajo de pedidos */}
<div style={{ padding:"12px 16px" }}>
<div style={{ background:CARD,border:`1px solid ${BORDER}`,borderRadius:10,padding:"12px" }}>
<div style={{ fontSize:9,color:"#555",fontFamily:"sans-serif",letterSpacing:"2px",marginBottom:10,textAlign:"center" }}>NUESTRAS REDES</div>
<div style={{ display:"flex",justifyContent:"center",gap:12 }}>
<a href="https://www.instagram.com/drparrillapy" target="_blank" rel="noreferrer" style={{ display:"flex",alignItems:"center",gap:6,textDecoration:"none",padding:"6px 12px",borderRadius:8,background:"linear-gradient(135deg,#833AB4,#FD1D1D,#F77737)" }}>
<svg viewBox="0 0 24 24" width="14" height="14" fill="#FFF"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
<span style={{ fontSize:9,color:"#FFF",fontFamily:"sans-serif",fontWeight:"bold" }}>Instagram</span>
</a>
<a href="https://www.facebook.com/doctorparrillaparaguay" target="_blank" rel="noreferrer" style={{ display:"flex",alignItems:"center",gap:6,textDecoration:"none",padding:"6px 12px",borderRadius:8,background:"#1877F2" }}>
<svg viewBox="0 0 24 24" width="14" height="14" fill="#FFF"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
<span style={{ fontSize:9,color:"#FFF",fontFamily:"sans-serif",fontWeight:"bold" }}>Facebook</span>
</a>
<a href="https://x.com/drparrillapy" target="_blank" rel="noreferrer" style={{ display:"flex",alignItems:"center",gap:6,textDecoration:"none",padding:"6px 12px",borderRadius:8,background:"#000",border:"1px solid #333" }}>
...
<span style={{ fontSize:9,color:"#FFF",fontFamily:"sans-serif",fontWeight:"bold" }}>X</span>
</a>
</div>
</div>
</div>
...
{misTickets.map(t => (
<div key={t.id} style={{ background:CARD,border:`1px solid ${BORDER}`,borderRadius:12,padding:"16px",marginBottom:10,position:"relative",overflow:"hidden" }}>
...
// ── ADMIN: CATÁLOGO ───────────────────────────────────────────────────────────
function AdminCatalog({ productos, setProductos, config, setConfig }) {
const [view, setView] = useState("list");
const [idx, setIdx] = useState(null);
...
<div style={{ padding:"16px",display:"flex",flexDirection:"column",gap:10 }}>
{/* ── CATÁLOGO PDF (ADMIN) ── */}
<div style={{ background:CARD, border:`1px solid ${GOLD}44`, borderRadius:12, padding:"16px", marginBottom:4 }}>
<div style={{ fontSize:11, color:GOLD, fontFamily:"sans-serif", letterSpacing:"2px", marginBottom:12 }}>📄 CATÁLOGO PDF GENERAL</div>
{config?.catalogoPdf ? (
<div>
<div style={{ display:"flex", alignItems:"center", gap:12, background:GOLD+"11", border:`1px solid ${GOLD}33`, borderRadius:10, padding:"12px 14px", marginBottom:10 }}>
<span style={{ fontSize:22 }}>📄</span>
<div style={{ flex:1 }}>
<div style={{ fontSize:13, color:GOLD, fontWeight:"bold", marginBottom:2 }}>{config.catalogoPdfNombre || "catalogo.pdf"}</div>
<div style={{ fontSize:11, color:"#666", fontFamily:"sans-serif" }}>Visible para todos los clientes ✅</div>
</div>
<a href={config.catalogoPdf} target="_blank" rel="noreferrer"
style={{ background:GOLD, border:"none", color:DARK, padding:"8px 14px", borderRadius:8, fontSize:12, fontFamily:"sans-serif", fontWeight:"bold", textDecoration:"none", flexShrink:0 }}>
Ver
</a>
</div>
<div style={{ display:"flex", gap:8 }}>
...
onPdf={(url, nombre) => setConfig(prev => ({ ...prev, catalogoPdf:url, catalogoPdfNombre:nombre }))}
style={{ flex:1, justifyContent:"center", boxSizing:"border-box" }}
/>
<button
onClick={() => { if(window.confirm('\u00bfEliminar el catálogo PDF? Los clientes ya no verán el botón de descarga.')) setConfig(prev => ({ ...prev, catalogoPdf:null, catalogoPdfNombre:null })); }}
style={{ background:"none", border:"1px solid #E5737444", color:"#E57373", padding:"10px 14px", borderRadius:10, fontSize:12, fontFamily:"sans-serif", cursor:"pointer" }}>
🗑 Quitar
</button>
</div>
</div>
...
<div>
<div style={{ fontSize:12, color:"#666", fontFamily:"sans-serif", marginBottom:10 }}>Subi el PDF del catálogo completo. Aparecerá un botón de descarga para todos los clientes.</div>
<PdfUploadButton
label="📄 Subir catálogo PDF"
onPdf={(url, nombre) => setConfig(prev => ({ ...prev, catalogoPdf:url, catalogoPdfNombre:nombre }))}
...
<div><div style={{ fontSize:10,color:"#555",fontFamily:"sans-serif",marginBottom:3 }}>INICIO DE OBRA</div><div style={{ fontSize:13,color:p.fechaInicio?GOLD:"#555",fontFamily:"sans-serif" }}>{p.fechaInicio||"Sin asignar"}</div></div>
  ...
  <div style={{ display:"flex",flexDirection:"column",gap:8,marginBottom:24 }}>
    {ESTADO_LABELS.map((est,i) => {
      ...
      const waMsg = `Hola ${cliente?.tratamiento ? cliente.tratamiento + " " + (cliente?.nombre?.split(" ").slice(-1)[0]||""): cliente?.nombre?.split(" ")[0]||""} 👋! Somos de *Doctor Parrilla* 🔥\n\nTe informamos que tu parrilla *${p.modelo}* (Pedido ${p.id}) ahora está en: *${est}*\n\n${i===0?"Ya recibimos tu pedido. Nuestro equipo ya está planificando cada detalle de tu parrilla.\n\nSeguí el progreso en tiempo real en: drparrillaparaguay.com ingresando con tu número de teléfono.": i===1?"Manos expertas están forjando tu parrilla ahora mismo. ¡Cada soldadura es perfección!\n\nMirá el avance en: drparrillaparaguay.com ingresando con tu número de teléfono.": i===2?"Tu parrilla está en control de calidad. Revisamos cada detalle para que sea perfecta. ¡Ya falta poco!\n\nMonitoreá el progreso en: drparrillaparaguay.com ingresando con tu número de teléfono.": i===3?"¡Tu parrilla está lista para entrega! 🎉 Coordinaremos la entrega pronto. ¿Ya compraste la carne para el estreno?\n\nSeguí los detalles en: drparrillaparaguay.com ingresando con tu número de teléfono.": "¡Tu parrilla ya fue entregada! 🎉 Disfrutala. Cada asado será un recuerdo imborrable.\n\n🎁 *REGALO EXCLUSIVO:* Dejá tu reseña en Google y recibí un *cupón de 10% de descuento* en servicios, mantenimiento o nuevos productos.\n\nIngresá a drparrillaparaguay.com con tu número de teléfono para reclamarlo."}\n\n#ElFuegoNosUne🔥 Gracias por confiar en Dr. Parrilla`;;
      return (
  ...
  {/* ── PRESUPUESTO PDF (ADMIN) ── */}
  <div style={{ background:CARD, border:`1px solid ${GOLD}44`, borderRadius:12, padding:"16px", marginBottom:20 }}>
    <div style={{ fontSize:11, color:GOLD, fontFamily:"sans-serif", letterSpacing:"2px", marginBottom:12 }}>📄 PRESUPUESTO PDF</div>
    {p.presupuestoPdf ? (
      <div>
        <div style={{ display:"flex", alignItems:"center", gap:12, background:GOLD+"11", border:`1px solid ${GOLD}33`, borderRadius:10, padding:"12px 14px", marginBottom:10 }}>
          <span style={{ fontSize:22 }}>📄</span>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:13, color:GOLD, fontWeight:"bold", marginBottom:2 }}>{p.presupuestoPdfNombre || "presupuesto.pdf"}</div>
            <div style={{ fontSize:11, color:"#666", fontFamily:"sans-serif" }}>PDF cargado ✅</div>
          </div>
          <a href={p.presupuestoPdf} target="_blank" rel="noreferrer"
            style={{ background:GOLD, border:"none", color:DARK, padding:"8px 14px", borderRadius:8, fontSize:12, fontFamily:"sans-serif", fontWeight:"bold", textDecoration:"none", flexShrink:0 }}>
            Ver
          </a>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <PdfUploadButton
            ...
            onPdf={(url, nombre) => setPedidos(pedidos.map((x,j) => j===selected ? {...x, presupuestoPdf:url, presupuestoPdfNombre:nombre} : x))}
            style={{ flex:1, justifyContent:"center", boxSizing:"border-box" }}
          />
          <button
            onClick={() => { if(window.confirm('\u00bfEliminar el presupuesto PDF de este pedido?')) setPedidos(pedidos.map((x,j) => j===selected ? {...x, presupuestoPdf:null, presupuestoPdfNombre:null} : x)); }}
            style={{ background:"none", border:"1px solid #E5737444", color:"#E57373", padding:"10px 14px", borderRadius:10, fontSize:12, fontFamily:"sans-serif", cursor:"pointer" }}>
            🗑 Quitar
          </button>
        </div>
      ...
      <div>
        <div style={{ fontSize:12, color:"#666", fontFamily:"sans-serif", marginBottom:10 }}>Subi el presupuesto en PDF para que el cliente lo vea en su pedido.</div>
        <PdfUploadButton
          label="📄 Subir presupuesto PDF"
          onPdf={(url, nombre) => setPedidos(pedidos.map((x,j) => j===selected ? {...x, presupuestoPdf:url, presupuestoPdfNombre:nombre} : x))}
          style={{ width:"100%", justifyContent:"center", boxSizing:"border-box" }}
        />
      </div>
...
// ── ADMIN: RESENAS─────────────────────────────────────────────────────────
function AdminResenas({ pedidos, clientes, cupones, setCupones }) {
...
{/* Individual reviews */}
  ...
  <div style={{ padding:"0 16px", display:"flex", flexDirection:"column", gap:10 }}>
    ...
    {resenas.sort((a,b) => b.stars-a.stars).map((r,i) => {
      // Buscar cupón pendiente de esta reseña
      const cuponPendiente = (cupones||[]).find(c => c.tipo==="resena" && normalizePhone(c.clienteTel||c.cliente||"")=== normalizePhone(r.clienteTel||r.pedidoId||"" ) && c.estado==="pendiente");
      const cuponActivo = (cupones||[]).find(c => c.tipo==="resena" && normalizePhone(c.clienteTel||c.cliente||"")=== normalizePhone(r.clienteTel||r.pedidoId||"" ) && c.estado==="activo");
      return (
      <div key={i} style={{ background:CARD, border:`1px solid ${starColor(r.stars)}44`, borderRadius:12, padding:"16px", position:"relative", overflow:"hidden" }}>
        <EmberSystem count={2} bottom={0} spread={60}/>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:8 }}>
          <div style={{ fontSize:11, color:"#555", fontFamily:"sans-serif" }}>{r.fecha}</div>
          {cuponActivo && (
            <div style={{ background:"#4CAF5022", border:"1px solid #4CAF5066", color:"#4CAF50", fontSize:11, padding:"4px 10px", borderRadius:20, fontFamily:"sans-serif", fontWeight:"bold" }}>
              ✅ Cupón activado
            </div>
          )}
          {cuponPendiente && setCupones && (
            <button onClick={() => {
              setCupones(prev => prev.map(c => c.id===cuponPendiente.id ? {...c, estado:"activo"} : c));
            }} style={{ background:`linear-gradient(135deg,${GOLD},${GOLD_LIGHT})`, border:"none", color:DARK, fontSize:12, padding:"6px 14px", borderRadius:20, fontFamily:"sans-serif", fontWeight:"bold", cursor:"pointer" }}>
              🎁 Activar cupón 10%
            </button>
          )}
        </div>
...
const localC = localRaw ? JSON.parse(localRaw) : [];
const fbIds = new Set(fbC.map(c => c.id));
const localOnly = localC.filter(c => c.id && !fbIds.has(c.id));
const merged = localOnly.length > 0 ? [...fbC, ...localOnly] : fbC;
setClientes(merged);
appStorage.set("dp_clientes", JSON.stringify(merged));
}
if (fbP && Array.isArray(fbP)) { setPedidos(fbP); appStorage.set("dp_pedidos", JSON.stringify(fbP)); }
...
try { await appStorage.set("dp_config",   JSON.stringify(config));    } catch(e) {}
}, 15000);
return () => clearTimeout(timer);
}, [clientes, pedidos, tickets, productos, visitas, cupones, config, storageReady]);
...
appStorage.set("dp_clientes", JSON.stringify(fbClientes));
return fbClientes;
});
...
"admin-catalog":  () => <AdminCatalog productos={productos} setProductos={saveProductos} config={config} setConfig={saveConfig} />,
